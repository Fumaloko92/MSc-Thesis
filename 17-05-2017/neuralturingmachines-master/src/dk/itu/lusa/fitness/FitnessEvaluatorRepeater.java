 ...
 *
 */
public class FitnessEvaluatorRepeater implements BulkFitnessFunction, Configurable {
	private static final long serialVersionUID = 1L;

	ActivatorTranscriber activatorFactory;
	private TuringControllerRepeater[] controllers;
	...
	public void init(Properties properties) {
		...
		if (threading) {
			...
			controllers = new TuringControllerRepeater[cores];
		} else if (threadPooling){
			threadPool = Executors.newCachedThreadPool();
			cachedController = loadController(properties);
		} else {
			cores = 1;
			controllers = new TuringControllerRepeater[1];
		}
		// Initialize
		generation = 0;
		...
		for (int i = 0; i < cores; i++) {
			controllers[i] = FitnessEvaluatorRepeater.loadController(properties);
		}
	}
	
	public static TuringControllerRepeater loadController(Properties props) {
		Simulator simulator = (Simulator) Utilities.instantiateObject(
				props.getProperty("simulator.class"),
				new Object[] { props }, null);
		TuringControllerRepeater result = new TuringControllerRepeater(props,simulator);
