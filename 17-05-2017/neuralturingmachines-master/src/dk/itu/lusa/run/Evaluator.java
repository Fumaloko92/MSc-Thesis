...
import dk.itu.lusa.turing.GravesTMWithTemporalLinks2;
...
//		ask(props,br,"simulator.copytask.length.max");
		...
		dk.itu.lusa.domain.blockpuzzle.Utilities.InitializeStaticVariables(props);
...
//		}
		...
		if(props.containsKey("threading") && props.getBooleanProperty("threading")) {
			final int numCores = Runtime.getRuntime().availableProcessors();
			results = new Collection[numCores];
			...
			threads[0] = new Thread(new Runnable() {
				public void run(){
					results[0] = handleSubset(0,numberOfTests,numberOfTests,props,chrom,progressCounter);
				}
			});
			threads[0].start();
		}
