...
public class MinimalTuringMachine implements TuringMachine, Replayable<MinimalTuringMachineTimeStep> {
    ...
    private boolean recordTimeSteps = true;
    ...
    private int memoryScreenshotIndex;
    ...
    public double[][] processInput(double[] fromNN) {
        ...
        int[] read_head = new int[pointers.length];
        int[] after_shift = new int[pointers.length];
        ...
        for (int i = 0; i < heads; i++) {
            ...
            System.arraycopy(pointers, 0, read_head, 0, pointers.length);

            System.arraycopy(pointers, 0, after_shift, 0, pointers.length);
...
//		return new double[1][result.length];
        if (MemoryScreenshot.PRINT_MEMORY_SCREENSHOTS) {
            MemoryScreenshot.AddMemoryToScreenshot(memoryScreenshotIndex, tape, read_head,write_head,after_shift);
        }
